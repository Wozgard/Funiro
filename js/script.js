...
const popupFall = document.querySelector('.popup__fall');
...
function popupOpen(currentPopup){
	if(currentPopup && unlock){
		const popupActive = document.querySelector('.popup__content.open');
		if(popupActive){
		...
		setTimeout(function() {
			popupFall.style.display = 'none';
		}, timeout);

		currentPopup.addEventListener('click', function(e){
			if(!e.target.closest('.popup__item') && !e.target.closest('.popup__fall')){ //заменить на .popup__content
				popupFall.style.display = 'flex';
				popupClose(e.target.closest('.popup__content'));//заменить на .popup
			}
		});
	}
}
...
document.addEventListener('keydown', function(e){
	if(e.which === 27){
		const popupActive = document.querySelector('.popup__content.open');//заменить на .popup.open
...
window.onload = function() {
	...
	function documentActions(e){
		...
		if(window.innerWidth > 768 && isMobile.any()){
			...
			if(!targetElement.closest('.menu__item') && document.querySelectorAll('.menu__item._hover').length > 0){
				$('.menu__item._hover').removeClass("_hover");
			}
		}
		...
		/*if(targetElement.classList.contains('row-furniture__item')){
			const popupName = targetElement.getAttribute('href');
			const currentPopup = document.getElementById(popupName);
	...
	function loadProducts(data){
		...
		data.products.forEach(item => {
			...
			if (productsLabels){ 
				var labels = '';
				productsLabels.forEach(labelsItem => {
					labels += `<div class="item-product__label item-product__label_${labelsItem.type}">${labelsItem.value}</div>`
				});
				var template = `
						<article data-pid="${productsId}" class="products__item item-product">
							<div class="item-product__labels">
								${labels}
							</div>
							<a href="${productsUrl}" class="item-product__image _ibg" style="background-image: url('img/products/${productsImage}.jpg');">
								<picture>
									<source srcset="img/products/${productsImage}.webp" type="image/webp">
									<img src="img/products/${productsImage}.jpg" alt="${productsTitle}">
								</picture>
							</a>
							<div class="item-product__body">
								...
								<div class="item-product__prices">
									<div class="item-product__price">${productsPrice}</div>
									<div class="item-product__price item-product__price_old">${productsPriceOld}</div>
								</div>
								<div class="item-product__actions actions-product">
									<div class="actions-product__body">
										<a href="" class="actions-product__button btn btn_white">Add to cart</a>
										<a href="${productsShareUrl}" class="actions-product__link _icon-share">Share</a>
										<a href="${productsLikeUrl}" class="actions-product__link _icon-heart">Like</a>
									</div>
								</div>
							</div>
						</article>
	...
	function addToCart(productButton, productId){
		if(!productButton.classList.contains('_hold')){
			...
			const productsImage = product.querySelector('.item-product__image');

			const productsImageFly = productsImage.cloneNode(true);

			const productsImageFlyWidth = productsImage.offsetWidth;
			const productsImageFlyHeight = productsImage.offsetHeight;
			const productsImageFlyTop = productsImage.getBoundingClientRect().top;
			const productsImageFlyLeft = productsImage.getBoundingClientRect().left;

			productsImageFly.setAttribute('class', '_flyImage _ibg');
			productsImageFly.style.cssText = 
			`
			left:${productsImageFlyLeft}px;
			top:${productsImageFlyTop}px;
			width:${productsImageFlyWidth}px;
			height:${productsImageFlyHeight}px;
			background-image: url("img/products/p${productId}.jpg");
			`;
	...
	function updateCart(productButton, productId, productAdd = true){
		...
		if(productAdd){
			...
			if(!cartProduct){
				...
				<a href="" class="cart-list__image _ibg" style="background-image: url('img/products/p${productId}.jpg');">${cartProductImage}</a>
	...
	if(furniture && !isMobile.any()){
		...
		function setMouseGalleryStyle() {
			...
			const furnitureDifferetn = furnitureItemsWidth - furniture.offsetWidth;
			const distX = Math.floor(coordXprocent - positionX);

			positionX = positionX + (distX * speed);
			let position = furnitureDifferetn / 200 * positionX;
		...
		furniture.addEventListener('mousemove', function(e){
			//Width
			const furnitureWudth = furniture.offsetWidth;

			//Null is center
			const coordX = e.pageX - furnitureWudth / 2;

			//Take procent
			coordXprocent = coordX / furnitureWudth * 200;
